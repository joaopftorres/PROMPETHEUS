...
def expand_title_gpt(title, model="gpt-3.5-turbo"):
...
def get_title_gpt(topic_keywords, model = "gpt-3.5-turbo"):
    completion = client.chat.completions.create(
        model=model,
        messages=[
...
def post_edit_gpt(summary, section_name, title, model = "gpt-3.5-turbo"):
  completion = client.chat.completions.create(
      model=model,
      messages=[
